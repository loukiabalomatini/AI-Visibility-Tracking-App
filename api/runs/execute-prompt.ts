...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { runId, promptId, provider = 'gemini' } = req.body || {};
    if (String(provider).toLowerCase() !== 'gemini') {
      return res.status(400).json({ error: `Only Gemini is currently enabled in this Vercel execution endpoint. Provider received: ${provider}` });
    }
    ...
    const currentPrompt = run.promptResults.find((p: any) =>
    ...
    const trackedBrands = [run.targetBrand, ...(run.competitors || [])];
    ...
    currentPrompt.brands = result.brands.map((b: any) => ({
      ...b,
      score: b.explicit_position === 1 ? 1 : b.explicit_position === 2 ? 0.8 : b.explicit_position === 3 ? 0.6 : b.explicit_position === 4 ? 0.4 : b.explicit_position === 5 ? 0.2 : b.mentioned ? 0.3 : 0,
    }));
    ...
    if (run.completedPrompts + run.failedPrompts === run.promptResults.length) {
